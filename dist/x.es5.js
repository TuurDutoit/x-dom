(function (global, factory) {
  ...
  var $test = create("p");
  var _matches = $test.matches || $test.matchesSelector || $test.webkitMatchesSelector || $test.mozMatchesSelector || $test.msMatchesSelector || $test.oMatchesSelector || matchesPolyfill;
  ...
  function descendants($elems, sel, all) {
    ...
    each($elems, function ($elem) {
      ...
      if (hasChildren) {
        $descendants.push.apply($descendants, toConsumableArray(descendants($elem, all, $descendants)));
      }
    });
  ...
  function on($elems, event, cb) {
    var opts = arguments.length <= 3 || arguments[3] === undefined ? false : arguments[3];
